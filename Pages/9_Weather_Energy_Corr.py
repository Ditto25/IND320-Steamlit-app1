...
with left_col:
    ...
    st.session_state["pricAarea"] = area
    ...
    prod_groups = sorted(
        df_prod.loc[df_prod["priceArea"] == area, "group"].dropna().unique() # <-- Endret til "group"
    )
    cons_groups = sorted(
        df_cons.loc[df_cons["priceArea"] == area, "group"].dropna().unique() # <-- Endret til "group"
    )
    ...
    if energy_mode == "Production":
        # Production har kun data fra 2022 og utover
        AVAILABLE_YEARS = [2022, 2023, 2024]
        group_options = prod_groups
        group_label = "Production group"
    else:
        # Consumption har data fra 2021 og utover
        AVAILABLE_YEARS = [2021, 2022, 2023, 2024]
        group_options = cons_groups
        group_label = "Consumption group"

    # 🎯 OPPDATERT: Bruker den dynamiske listen AVAILABLE_YEARS
    year = st.selectbox(
        "Year",
        AVAILABLE_YEARS, # Bruker dynamisk liste her
        index=len(AVAILABLE_YEARS) - 1, # Setter standardverdi til siste år (2024)
        help="Year for both weather and energy series.",
    )
...
if energy_mode == "Production":
    df_energy = df_prod[
        ...
        & (df_prod["group"] == energy_group) # <-- Endret til "group"
