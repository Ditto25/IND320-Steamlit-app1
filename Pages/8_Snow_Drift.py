...
from utils.Data_loader import load_open_meteo_api
...
def download_weather_for_seasons(
    ...
    for year in years:
        ...
        # ⚠️ MERK: Denne funksjonen må være implementert i utils/Data_loader.py!
...
def plot_wind_rose(avg_sector_values: np.ndarray, overall_avg_kgm: float):
    ...
    fig = go.Figure(
        data=go.Barpolar(
            ...
            theta=theta_deg,
            text=directions,
            # Formatering av hovertekst for å vise 3 desimaler
...
def main() -> None:
    ...
    with st.expander("Snow transport model parameters", expanded=False):
        # 🎯 MERK: T, F, theta er parametre for Tabler (2003)
        T = st.number_input(
    ...
    with st.spinner("Downloading hourly weather data from Open-Meteo ..."):
        try:
            # Bruker den cached download_weather_for_seasons (tilpasset)
    ...
    st.dataframe(
        ...
        # Viser kun rader, ikke Pandas-indeksen
        hide_index=True, 
    )
