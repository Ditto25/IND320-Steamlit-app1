...
# -----------------------------
# I filen: Pages/7_Map_Visualization.py
def get_groups(kind: str) -> list[str]:
    ...
    # NÅ BRUKER VI DET KORREKTE NAVNET 'group'
    field = "group" 
    
    if df.empty or field not in df.columns:
        return []

    # Bruk .str.lower() for å normalisere strenger hvis nødvendig, 
    # men vi kan starte med å bare bruke unike verdier
    groups = sorted(df[field].dropna().unique())
    return groups

# I Pages/7_Map_Visualization.py
...
def mean_by_pricearea(
    ...
    # Bruk riktige kolonnenavn (startTime, priceArea, quantityKwh, group)
    mask = (
        (df[group_field] == group) # Bruker "group" her
        & (df["startTime"] >= start_ts)
        & (df["startTime"] < end_ts)
    )
    
    # Hent ut kolonner med CamelCase
    df_sel = df.loc[mask, ["priceArea", "quantityKwh"]]

    if df_sel.empty:
        return pd.DataFrame(columns=["priceArea", "mean_kwh"])

    # ... (resten av funksjonen er OK)
    agg = (
...
def main():
    ...
    if df_mean.empty:
       ...
       df_mean['mean_gwh'] = df_mean['mean_kwh'] / 1_000_000
       dynamic_col_name = "Mean GWh for {group}".format( # Endret til GWh
