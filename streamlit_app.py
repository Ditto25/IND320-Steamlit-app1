...
def discover_pages():
    ...
    candidates = []
    # Antar at Page-mappen ligger i samme nivå som app-filen
...
def emoji_for(display: str) -> str:
    mapping = {
        ...
        'video': '🎬',
        'timeline': '📅',
        'elhub': '🔌', # Lagt til for Elhub-data
        'stl': '📉',   # Lagt til for STL
        'forecast': '📈',
        'outliers': '🔬', # Lagt til for Outliers
        'weather': '🌦️'  # Lagt til for Weather
    }
    low = display.lower()
...
# --------------------------------------------------------
# 🧭 INITIALISERING OG NAVIGASJON I SIDEFELTET
# --------------------------------------------------------

# 1. OPPDAG SIDER OG BYGG KART
pages = discover_pages()

# Bygg en palett og tilordne en emoji til hver oppdaget side
emoji_palette = [
    "📈", "🎯","📊", "🔬", "🔍"
]
emoji_map = {}
for i, (_mod, display, path) in enumerate(pages):
    # Bruk den generiske emoji-funksjonen for bedre konsistens
    emoji_map[path] = emoji_for(display) 
...
# Navigasjon for alle oppdagede sider
for mod_name, display, path in pages:
    # Hent emoji fra det forhåndsdefinerte kartet
...
if current == "Home":
    ...
    if pages:
        for _mod, display, path in pages:
            # Henter emoji fra det forhåndsdefinerte kartet
            emoji = emoji_map.get(path, emoji_for(display))
